...
| `modelConfigRules` | `{modelRules[], modelApiRules[], providerSiteRules[], templateModelRules[], builtinProviderModelRules[], providerModelRules[], manualProviderModelRules[]}`；注意这 7 个数组**分属两组 strict schema**：builtin 组（`builtinModelConfigRulesSchema`）= modelRules/modelApiRules/providerSiteRules/templateModelRules/builtinProviderModelRules，personal 组（`personalModelConfigRulesSchema`）= providerModelRules/manualProviderModelRules；个人渠道只写 `providerModelRules`（必要时 `manualProviderModelRules`） |
...
| 模型 `config.properties` | 可选对象（sparse：每个叶子均可省略/null）：`contextWindow`（正整数）、`inputFormat: {supportsText, supportsImage, supportsVideo, supportsAudio, supportsPdf}`、`outputFormat: {supportsText}`、`supportsToolCall`、`supportsJsonSchemaOutput`、`supportsNativeWebSearch`、`supportsMidConversationSystem`、`requiresMfjsToolSchema`（均 boolean） |
| 模型 `config.optionSpecs` | 可选对象：`reasoningLevel: {values: string[]（非空、去重、非空白，按语义强度从低到高排列，首项为最低公开档位）, map: string}`、`maxOutputTokens: {max: 正整数, map: string}`；**`map` 为受限 CEL 表达式字符串**（`@zcode/model-option-map` 编译，语法非法即校验失败），内置配置用 `"{}"`，完整 schema 必填、sparse 形式可省略 |
| `manualProviderModelRules[]` | 手动模式字段白名单（`manual-model-config.ts`）：`enabled`；`properties` 仅 `contextWindow`、`supportsJsonSchemaOutput`、`supportsNativeWebSearch`、`supportsMidConversationSystem`、`inputFormat: {supportsImage, supportsVideo, supportsPdf}`；`optionSpecs` 仅 `reasoningLevel`（含 map）与 `maxOutputTokens: {max}`。同一 `providerId+modelId` 不能同时出现在 `providerModelRules`（智能）和 `manualProviderModelRules`（手动）中 |
| `providerRules[]` 条目可选字段 | `{providerId, templateId?, providerName?, enabled?, config}`——`templateId`/`enabled` 为可选；`providerId` 在同一文件内不可重复；以 `account:` 开头的 providerId **不允许声明 `config.access`**（只能由内置配置声明） |
| `config` 顶层可选字段 | `providerOrder?`（非空字符串数组）、`defaultModelSelection?`（`@zcode/shared/model-selection`）；文件编码为 `{schemaVersion: 1, config}`，`schemaVersion` 高于 1 的文件被拒绝，低版本经迁移链升级（`provider-node/provider-config-file-codec.ts`，当前版本常量 `CURRENT_SCHEMA_VERSION = 1`） |
| `apiKeyManagementUrl` | 可选，必须是合法 URL（如 `https://z.ai/manage-apikey/apikey-list`） |
| api 端点路径（`api.type` → 请求路径） | `anthropic-messages` → `/v1/messages`；`openai-chat-completions` → `/chat/completions`；`openai-responses` → `/responses`（UI `ProviderApiFormatSelect.tsx`）；personal 渠道的 `api.baseUrl` 允许暂存为空串/任意字符串（草稿态），但完整校验必须是合法 URL |
