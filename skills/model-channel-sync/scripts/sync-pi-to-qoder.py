#!/usr/bin/env python3
"""pi → qoder 全量同步（渠道独立式：每渠道独立 provider、独立密钥、模型按渠道归属）。

用法:
    python3 scripts/sync-pi-to-qoder.py

**双目录同步**：qoder 是同一软件的国外版（`~/.qoder`）与国内版（`~/.qoder-cn`），
...
def sync(qoder_path, pi, upstream_ids, upstream_short_ids, channel_ids, baseurl_to_channel):
    """对单个 settings.json 执行同步（幂等；备份+断言校验）"""
    ...
    for pname, pdata in pi['providers'].items():
        ...
        if base:
            same = [mm for mm in model_ids if fam_key(mm) == fam_key(base)]
    ...
    print(f'== {qoder_path}: 校验通过（渠道独立：baseUrl 强绑定 / 自动创建；仅 models 增迁 / apiKey / displayName 变更）')
    print('\n'.join(out))
...
def main():
    ...
    # 上游免费渠道的模型 id 索引（这些模型归对应渠道 provider 所有，参与归属判定时豁免）——只取一次，供各目录共用
    upstream_ids = {}
    ...
    for d in QODER_DIRS:
        sync(os.path.join(os.path.expanduser(d), 'settings.json'),
             pi, upstream_ids, upstream_short_ids, channel_ids, baseurl_to_channel)
